...
Откройте Docker, перейдите в Settings, далее в Resouses, далее в Advanced. Убедитесь, что докеру отводится минимум 3 гб от ресурсов вашего компьютера (если настройка невозможна, то существует возможность изменить конфигурации в файле wclconfig).
...
Далее перейдите в раздел Install в шапке сайта.
...
Следующим этапом перед загрузкой контейнеров необходимо задать конфигурации для ElasticSearch:
...
Ожидайте скачивание и запуск контейнеров. Это может занять около 30 мин (зависит от скорости вашего соединения).

После того как произошла установка всех контейнеров в конце окна терминала вы увидите следующие логи, подтверждающие успешность загрузки и запуска контейнеров (см. ниже).
...
Примечание: работают все контейнеры, кроме одного контейнера, который выполняет один скрипт и закрывается.
...
Если этого не произошло, то введите иной URL: http://localhost:8080

Войдите от имени администратора. Введите логин: superadmin. Введите пароль: erebus

Примечание: В последующим рекомендуется заменить пароль для обеспечения безопасности.

Нажмите на кнопку Login, вид страницы при успешном входе представлен на следующем изображении:
...
Примечание: В данном руководстве представлена интеграция ReportPortal в уже существующий проект. Кроме того, данное руководство по интеграции значительно отличается от инструкции разработчиков.

В папке resources (в дериктории test) создайте поддиректорий:  /META-INF/services

Примечание: в дальнейших шагах будет упоминаться именно эта папка resources.

Добавьте в этот поддиректорий файл с названием: org.junit.jupiter.api.extension.Extension.
...
В раздел "repositories" дополнительно внесите:
...
Для этого создайте тестовый класс LoggingUtils, поместите его папку util (заранее создайте ее) и добавьте в класс следующий код:
...
В папке resourced также cоздайте файл logback.xml и поместите в него следующую информацию:
...
Далее нажмите Add New Project для создания нового проекта.
...
Далее в папке resources создайте файл с названием reportportal.properties и скопируйте в него содержание блока CONFIGURATION EXAMPLES (Java). Это позволит прявязать ваш тестовый проект к "аккаунту" ReportPortal.
...
Преобразуте заданную по умолчанию строку "rp.launch" в reportportal.properties следующим образом: 
`rp.launch = ChangeOfTimeTest`
...
Теперь перейдите в класс, в котором непосредственно находятся ваши тесты, и перед публичным классом укажите аннотацию, которая позволит сделать тест отслеживаемым ReportPortal при помощи ранее созданного вспомогательного класса ScreenShooterReportPortalExtension.java:
...
Добавьте импорты в тот же класс (в нем находятся ваши тесты):
